...
export default function MyPageNavigator({auth, isMyPage}: Props) {
	...
	return (
		<S.Wrapper>
			<S.Tab>
				<Link to={`/profile/${profileId}`}>소개</Link>
			</S.Tab>

			{ auth === 'expert' &&
				<>
					<S.Tab>
						<Link to={`/profile/${profileId}/portfolios?page=1`}>포트폴리오</Link>
					</S.Tab>
					<S.Tab>
						<Link to={`/profile/${profileId}/reviews?page=1`}>리뷰</Link>
					</S.Tab>
				</>
			}
			...
			{ isMyPage &&
				...
				<S.Tab>
					<Link to={`/profile/${profileId}/bookmarks?page=1`}>북마크</Link>
				</S.Tab>
				</>
			}
		</S.Wrapper>
